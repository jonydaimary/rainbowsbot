...
if (command === 'h') {
    message.delete();
    let embed = new Discord.RichEmbed()
    .addField(`СИСТЕМА ОПЫТА`, `По мере получения новых уровней Вы можете получить роли, с более полезными правами\n\nНиже представлены требования для получения ролей\n\n**Новичок** - 0 уровень\n**Поселенец** - 3 уровень\n**Местный** - 6 уровень\n**Старейшина** - 10 уровень\n\nУровень можно повысить проявляя активность в чате. Для того, чтобы избежать спама, бот засчитывает только одно сообщение в минуту. При получении уровня бот отправляет Вам в чат сообщение об этом`)
    .setColor(`#800080`)
    .setFooter(`Администрация | 11.07.2018`);
